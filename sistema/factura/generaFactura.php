...
<?php
	...
	if(empty($_REQUEST['cl']) || empty($_REQUEST['f']))
	{
		...
		while ($row = mysqli_fetch_assoc($productos)) {
			...
			$pdf->Cell(15, 5, number_format($row['precio'], 0, '.', ','), 0, 0, 'L');
		...
		$pdf->Cell(76, 5, 'Total: ' . number_format($result_venta['totalfactura'], 0, '.', ','), 0, 1, 'R');
		$pdf->Ln();
		$pdf->SetFont('Arial', '', 7);
		$pdf->Cell(80, 5, utf8_decode("Gracias por su compra - Artículo con IVA incluido"), 0, 1, 'C');
